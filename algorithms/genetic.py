...
def run_genetic_algorithm(pop_size: int, gene: int, tour: int, cross: float, mut: float, flowers_file: str, bins_file: str, results_path: str, iterations: int):
    for i in range(iterations):
        file_jpg=f"p{pop_size}_g{gene}_c{cross}_m{mut}_t{tour}_i{i}.jpg"
        file_csv = f"p{pop_size}_g{gene}_c{cross}_m{mut}_t{tour}_i{i}.csv"
        genetic_config = AlgorithmConfig(
            ...
            results_file=os.path.join(results_path, file_csv),
            results_file2=os.path.join(results_path, file_jpg),
            bins_file=bins_file,
        )
